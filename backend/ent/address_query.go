...
//		GroupBy(address.FieldPrefecture).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Address.Query().
//		Select(address.FieldPrefecture).
//		Scan(ctx, &v)
...
func (aq *AddressQuery) loadBusiness(ctx context.Context, query *BusinessQuery, nodes []*Address, init func(*Address), assign func(*Address, *Business)) error {
	...
	for i := range nodes {
		if nodes[i].business_address == nil {
			continue
		}
		fk := *nodes[i].business_address
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "business_address" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
