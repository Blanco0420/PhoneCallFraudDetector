...
func (cq *CommentQuery) QueryProvider() *ProviderQuery {
	query := (&ProviderClient{config: cq.config}).Query()
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withProvider: cq.withProvider.Clone(),
...
func (cq *CommentQuery) WithProvider(opts ...func(*ProviderQuery)) *CommentQuery {
	query := (&ProviderClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withProvider = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withProvider; query != nil {
		if err := cq.loadProvider(ctx, query, nodes, nil,
			func(n *Comment, e *Provider) { n.Edges.Provider = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadProvider(ctx context.Context, query *ProviderQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Provider)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].provider_comment == nil {
			continue
		}
		fk := *nodes[i].provider_comment
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "provider_comment" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
